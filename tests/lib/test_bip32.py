...
mpubkey, mpubver = bip32.from_extended_key_string(MXPUB)
mprivkey, mprvver = bip32.from_extended_key_string(MXPRV)
...
def test_from_extended_key():
    ...
    with pytest.raises(ValueError):  # invalid exponent
...
class TestPubKey(object):
    ...
    def test_constructor(self):
        ...
        cls(raw_pubkey, chain_code, 0, 255, mpubkey.fingerprint())
    ...
    def test_from_extended_key_string(self):
        assert mpubver == bytes.fromhex("0488b21e")
        assert mpubkey.n == 0
    ...
    def test_extended_key_string(self):
        ...
        assert mpubkey.extended_key_string(mpubver) == MXPUB
        chg_master = mpubkey.child(1)
        chg5 = chg_master.child(5)
        assert chg5.address(0) == '1BsEFqGtcZnVBbPeimcfAFTitQdTLvUXeX'
        assert chg5.extended_key_string(mpubver) == 'xpub6AzPNZ1SAS7zmSnj6gakQ6tAKPzRVdQzieL3eCnoeT3A89nJaJKuUYWoZuYp8xWhCs1gF9yXAwGg7zKYhvCfhk9jrb1bULhLkQCwtB1Nnn1'

        ext_key_base58 = chg5.extended_key_string(mpubver)
        ...
        # Check can recreate
        dup, ver = bip32.from_extended_key_string(ext_key_base58)
        assert ver == mpubver
    ...
    def test_child(self):
        ...
        assert rec_master.address(0) == '18zW4D1Vxx9jVPGzsFzgXj8KrSLHt7w2cg'
        chg_master = mpubkey.child(1)
        assert chg_master.parent_fingerprint == mpubkey.fingerprint()
        assert chg_master.address(0) == '1G8YpbkZd7bySHjpdQK3kMcHhc6BvHr5xy'
        rec0 = rec_master.child(0)
        assert rec0.address(0) == '13nASW7rdE2dnSycrAP9VePhRmaLg9ziaw'
        rec19 = rec_master.child(19)
        assert rec19.address(0) == '15QrXnPQ8aS8yCpA5tJkyvXfXpw8F8k3fB'
        chg0 = chg_master.child(0)
        assert chg0.parent_fingerprint == chg_master.fingerprint()
        assert chg0.address(0) == '1L6fNSVhWjuMKNDigA99CweGEWtcqqhzDj'
    ...
    def test_address(self):
        assert mpubkey.address(0) == '1ENCpq6mbb1KYcaodGG7eTpSpYvPnDjFmU'
...
class TestPrivKey(object):
    ...
    def test_address(self):
        assert mprivkey.address(0) == mpubkey.address(0)
    ...
    def test_from_extended_key_string(self):
        ...
        assert mprvver == bytes.fromhex("0488ade4")
    ...
    def test_extended_key_string(self):
        ...
        assert mprivkey.extended_key_string(mprvver) == MXPRV
        chg_master = mprivkey.child(1)
        chg5 = chg_master.child(5)
        assert chg5.WIF(128) == 'L5kTYMuajTGWdYiMoD4V8k6LS4Bg3HFMA5UGTfxG9Wh7UKu9CHFC'
        ext_key_base58 = chg5.extended_key_string(mprvver)
    ...
    def test_child(self):
        ...
        assert rec_master.address(0) == '18zW4D1Vxx9jVPGzsFzgXj8KrSLHt7w2cg'
        chg_master = mprivkey.child(1)
        assert chg_master.parent_fingerprint == mprivkey.fingerprint()
        assert chg_master.address(0) == '1G8YpbkZd7bySHjpdQK3kMcHhc6BvHr5xy'
        rec0 = rec_master.child(0)
        assert rec0.WIF(128) == 'L2M6WWMdu3YfWxvLGF76HZgHCA6idwVQx5QL91vfdqeZi8XAgWkz'
        rec19 = rec_master.child(19)
        assert rec19.WIF(128) == 'KwMHa1fynU2J2iBGCuBZxumM2qDXHe5tVPU9VecNGQv3UCqnET7X'
        chg0 = chg_master.child(0)
        assert chg0.parent_fingerprint == chg_master.fingerprint()
        assert chg0.WIF(128) == 'L4J1esD4rYuBHXwjg72yi7Rw4G3iF2yUHt7LN9trpC3snCppUbq8'
...
class TestVectors():
    ...
    def test_vector1(self):
        ...
        xprv = m1.extended_key_string(mprvver)
        ...
        xpub = m1.public_key.extended_key_string(mpubver)
        ...
        # Chain m/0H/1
        m2 = m1.child(1)
        xprv = m2.extended_key_string(mprvver)
        ...
        xpub = m2.public_key.extended_key_string(mpubver)
        ...
        xprv = m3.extended_key_string(mprvver)
        ...
        xpub = m3.public_key.extended_key_string(mpubver)
        ...
        # Chain m/0H/1/2H/2
        m4 = m3.child(2)
        xprv = m4.extended_key_string(mprvver)
        ...
        xpub = m4.public_key.extended_key_string(mpubver)
        ...
        xprv = m5.extended_key_string(mprvver)
        ...
        xpub = m5.public_key.extended_key_string(mpubver)
